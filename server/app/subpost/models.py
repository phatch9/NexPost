...
class Subpost(db.Model):
    ...
    user = db.relationship("User", back_populates="subpost")
    ...
    def as_dict(self, cur_user_id=None):
        ...
        if cur_user_id:
            data["has_subscribed"] = bool(
                Subscription.query.filter_by(user_id=cur_user_id, subpost_id=self.id).first()
            )
        return data
...
class Subscription(db.Model):
    ...
    subpost_id = db.Column(db.Integer, db.ForeignKey("subposts.id"), nullable=False)
    user = db.relationship("User", back_populates="subscription")
    subpost = db.relationship("Subpost", back_populates="subscription")

    @classmethod
    def add(cls, post_id, user_id):
        new_sub = Subscription(user_id=user_id, subpost_id=post_id)
        db.session.add(new_sub)
        db.session.commit()

    def __init__(self, user_id, subpost_id):
        self.user_id = user_id
        self.subpost_id = subpost_id


class SubpostInfo(db.Model):
    __tablename__ = "subpost_info"
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    name = db.Column(db.Integer, db.ForeignKey("subposts.name"))
    logo = db.Column(db.Text)
    members_count = db.Column(db.Integer)
    posts_count = db.Column(db.Integer)
    comments_count = db.Column(db.Integer)
    subpost = db.relationship("Subpost", back_populates="subpost_info")
